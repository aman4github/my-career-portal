...
const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      const showLoading = () => {
                  Swal.fire({
                      text: 'Please Wait... We are fetching your data and generating OTP for You...Do not Refresh or Back the Page',
                      icon: 'warning',
        ...
        const response = await axios.post(
            "http://localhost:8090/management/admin/adminlogin",
            {
                email,
                password
            }
...
const verifyOtp = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            `http://localhost:8090/management/admin/verifyotp?email=${email}&otp=${otp}`
        );
  ...
  return (
    ...
    !otpSent ? (
        ...
        <button
            disabled={loading}
            className="w-full py-4 rounded-2xl font-black text-lg bg-red-600 text-white flex items-center justify-center gap-2"
        >
            {loading ? (
        ...
        <div className="space-y-4">
          ...
          <p className="mt-8 text-center font-bold text-gray-600">Login as a User? <Link to="/login-user" className="text-blue-600 hover:underline">Click Here</Link></p>
        </div>
